...
const Footer = () => {
    return (
        <div className="pt-20 pb-5 flex justify-around  bg-mine-shaft-800 font-['Poppins']">
            <div className="w-1/4 flex flex-col gap-4">
                <div className='flex gap-1 items-center text-bright-sun-400'>
                    <IconAnchor className="h-6 w-6" stroke={2.5} />
                    <div className="text-xl font-semiBold">JobHook</div>
                </div>
                <div className="text-sm text-mine-shaft-300 ">Job portal with user profiles, skill updates,
                ...
                [&>div]:p-2 [&>div]:rounded-full [&>div]:cursor-pointer mb-1 hover:[&>div]:bg-mine-shaft-700">
            ...
            </div>
            {
                footerLinks.map((item, index) => <div key={index}>
                    <div className="text-lg font-semibold mb-4 text-bright-sun-400">{item.title}</div>
                    {
                        ...
                        hover:text-bright-sun-400 cursor-pointer hover:translate-x-2 transition duration-300 ease-in-out">
                            {link}</div>)
                    }
